...
The payout multiplier to be hit. Either an exact number, or an inclusive `[min, max]` range.

Comparisons use **0.01x precision** (the same cent units as book / LUT `payoutMultiplier`), so exact targets like `multiplier: 5` match wallet totals that round to `5.00x`. Prefer ranges for hard-to-hit pays — exact values still retry until hit.
