...
```ts lineNumbers
import { createSlotGame, GameConfig } from "@slot-engine/core"
...
```ts lineNumbers
import { createSlotGame, GameConfig } from "@slot-engine/core" // [!code --]
...
Finally, [simulate](/docs/core/simulation) your game, tweak settings to improve the feel of it,
and [optimize](/docs/core/optimization) results to achieve your desired RTP.

## `createSlotGame()` Options [#options]

### `id`: string

A unique identifier for your game.
...
The maximum bet multiplier payout. Wins exceeding this number will be capped.

### `gameModes`: Record\<string, GameMode\>

See: [Game Configuration: Game Modes](/docs/core/config/game-modes)

### `symbols`: Record\<string, GameSymbol\>

See: [Game Configuration: Symbols](/docs/core/config/symbols)

### `padSymbols`: number

Amount of padding symbol rows above and below the active board.
...
See: [Game Configuration: User State](/docs/core/config/custom-state)

### `hooks`: Record\<string, (ctx) => unknown>

See: [Game Configuration: Hooks](/docs/core/config/hooks)