...
<Callout title="Max win must stay hittable">
If tilt or `scale` rules would assign weight `0` to every book that pays the global maximum,
optimization **throws** instead of writing an unhittable max-win LUT. Give max-win books their
own `hitRate` target (recommended) rather than burying them in a heavily tilted criteria.
</Callout>
